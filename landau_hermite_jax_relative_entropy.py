...
def make_linearized_rhs_1sp_jax(rhs11_jit, fM):
    ...
    def L(h):
        ...
        return L_apply_flat(fM.reshape(-1), h.reshape(-1)).reshape(p, p, p)
...
def main() -> None:
    ...
    ap.add_argument("--nmax", type=int, default=6, help="Maximum Hermite mode index per velocity dimension.")
    ...
    ap.add_argument("--amp1", type=float, default=1.4, help="Fig1 1sp control: for prl_m2 it's the 2nd-moment anisotropy amplitude; for twostream it's the stream separation u in vx/vth; for poly4 it's an anisotropy knob.")
