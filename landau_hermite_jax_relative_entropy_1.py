...
def _lb_mode_degree_tensor(p: int) -> np.ndarray:
    ii, jj, kk = np.indices((int(p), int(p), int(p)), dtype=np.float64)
    return ii*(ii-1)*(ii-2) + jj*(jj-1)*(jj-2) + kk*(kk-1)*(kk-2)


def lb_collision_np(f: np.ndarray, nu_LB: float) -> np.ndarray:
    nu = float(nu_LB)
    if abs(nu) <= 0.0:
        return np.zeros_like(f)
    deg = _lb_mode_degree_tensor(int(f.shape[0]))
    return -(nu * deg) * np.asarray(f, dtype=np.float64)


def build_lb_collision_jax(nmax: int, nu_LB: float):
    jax, jnp = _maybe_import_jax("jax")
    assert jax is not None and jnp is not None
    nu = float(nu_LB)
    deg = jnp.asarray(_lb_mode_degree_tensor(int(nmax) + 1), dtype=jnp.float64)
    nu_j = jnp.asarray(nu, dtype=jnp.float64)

    def lb(f):
        return -(nu_j * deg) * jnp.asarray(f, dtype=jnp.float64)
...
def apply_kronecker_3_np_batched(Mx: np.ndarray, My: np.ndarray, Mz: np.ndarray, f: np.ndarray) -> np.ndarray:
    """Batched (over leading dimension q) Kronecker application.

    Mx, My, Mz: (Q, k, p)
    f: (p, p, p)
    Returns: (Q, k, k, k)
    """
    tmp = np.einsum("qka,abc->qkbc", Mx, f, optimize=True)
    tmp = np.einsum("qlb,qkbc->qklc", My, tmp, optimize=True)
    out = np.einsum("qmc,qklc->qklm", Mz, tmp, optimize=True)
    return out



def einsum_mpo_dot_np(g: np.ndarray, S: np.ndarray, Px: np.ndarray, Py: np.ndarray, Pz: np.ndarray) -> np.ndarray:
    """Fast NumPy contraction for the MPO dot."""
    tmp = np.tensordot(Px, g, axes=(1, 0))          # (x, d, b, c)
    tmp = np.tensordot(Py, tmp, axes=(1, 2))        # (y, e, x, d, c)
    tmp = np.tensordot(Pz, tmp, axes=(1, 4))        # (z, f, y, e, x, d)
    tmp = np.transpose(tmp, (4, 2, 0, 5, 3, 1))     # (x, y, z, d, e, f)
    out = np.tensordot(tmp, S, axes=([3, 4, 5], [0, 1, 2]))
    return out
...
def build_S_np(fb: np.ndarray, T: ModelTablesNP) -> Tuple[np.ndarray, np.ndarray]:
    """Build S1,S2 of shape (3,3,p_kp,p_kp,p_kp) from fb (p,p,p) using batched q."""
    fb = np.asarray(fb, dtype=np.float64)
    S1 = np.zeros((3, 3, T.p_kp, T.p_kp, T.p_kp), dtype=np.float64)
    S2 = np.zeros_like(S1)
    wq = np.asarray(T.wq_prefI, dtype=np.float64)

    for i in range(3):
        for j in range(3):
            t0 = apply_kronecker_3_np_batched(
                T.M_buildS[:, i, j, 0, 0],
                T.M_buildS[:, i, j, 0, 1],
                T.M_buildS[:, i, j, 0, 2],
                fb,
            )
            t1 = apply_kronecker_3_np_batched(
                T.M_buildS[:, i, j, 1, 0],
                T.M_buildS[:, i, j, 1, 1],
                T.M_buildS[:, i, j, 1, 2],
                fb,
            )
            S1[i, j] = np.tensordot(wq, (t0 + t1), axes=(0, 0))

            t2 = apply_kronecker_3_np_batched(
                T.M_buildS[:, i, j, 2, 0],
                T.M_buildS[:, i, j, 2, 1],
                T.M_buildS[:, i, j, 2, 2],
                fb,
            )
            t3 = apply_kronecker_3_np_batched(
                T.M_buildS[:, i, j, 3, 0],
                T.M_buildS[:, i, j, 3, 1],
                T.M_buildS[:, i, j, 3, 2],
                fb,
            )
            S2[i, j] = np.tensordot(wq, (t2 + t3), axes=(0, 0))
...
def make_linearized_rhs_1sp_numpy(T: ModelTablesNP, fM: np.ndarray, *, use_tt: bool, tt_tol: float, tt_rmax: int, nu_LB: float = 0.0):
    """
    Return L(h) = Q(h,fM) + Q(fM,h) using the same fast SOE→MPO machinery.
    """
    fM = np.asarray(fM, dtype=np.float64)
    S1M, S2M = build_S_np(fM, T)
    lb_deg = _lb_mode_degree_tensor(int(fM.shape[0]))
    nu = float(nu_LB)
...
def make_linearized_rhs_2sp_numpy(
    ...
    *,
    ...
    lb_deg_a = _lb_mode_degree_tensor(int(faM.shape[0]))
    lb_deg_b = _lb_mode_degree_tensor(int(fbM.shape[0]))
    nu = float(nu_LB)
    ...
    def J_apply(dfa: np.ndarray, dfb: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        out_a = term_a1 + term_a2
        out_b = term_b1 + term_b2
        if abs(nu) > 0.0:
            out_a = out_a - (nu * lb_deg_a) * dfa
            out_b = out_b - (nu * lb_deg_b) * dfb
        return out_a, out_b

    return J_apply


def make_linearized_rhs_1sp_jax(rhs11_jit, fM, nu_LB: float = 0.0):
    jax, jnp = _maybe_import_jax("jax")
    assert jax is not None and jnp is not None

    fM = jnp.asarray(fM, dtype=jnp.float64)
    p = fM.shape[0]
    lb_deg = jnp.asarray(_lb_mode_degree_tensor(int(p)), dtype=jnp.float64)
    nu = jnp.asarray(float(nu_LB), dtype=jnp.float64)
    ...
    def L_apply_flat(fM_flat, h_flat):
    ...
    def L(h):
        ...
        out = L_apply_flat(fM.reshape(-1), h.reshape(-1)).reshape(p, p, p)
        return out - (nu * lb_deg) * h

    return jax.jit(L)


def make_linearized_rhs_2sp_jax(rhsab_jit, rhsba_jit, faM, fbM, nu_LB: float = 0.0):
    jax, jnp = _maybe_import_jax("jax")
    assert jax is not None and jnp is not None
    ...
    def J_apply_flat(y0_flat, dy_flat):
        return jax.jvp(rhs_pair_flat, (y0_flat,), (dy_flat,))[1]
    ...
    def J_apply(dfa, dfb):
        ...
        out = J_apply_flat(y0, dy)
        dfa_out = out[:Na].reshape(p, p, p)
        dfb_out = out[Na:].reshape(p, p, p)
        return dfa_out - (nu * lb_deg) * jnp.asarray(dfa, dtype=jnp.float64), dfb_out - (nu * lb_deg) * jnp.asarray(dfb, dtype=jnp.float64)
...
def rel_entropy_KL_local_maxwellian_grid_tensor_precomp(
    ...
    nmax = int(grid.nmax)
    fxyz_raw = np.einsum("nmp,nx,my,pz->xyz", f, grid.psi, grid.psi, grid.psi, optimize=True)
    fxyz = np.maximum(fxyz_raw, eps)
...
def make_fig1_panel(
    *,
    ...
    gs = fig.add_gridspec(2, 2, left=0.07, right=0.995, bottom=0.05, top=0.95, wspace=0.30, hspace=0.25)
    ...
    if D1_ratio_lin is not None:
        axa.plot(t1, D1_ratio_lin, color="C2", ls="--", label=r"$\mathcal{D}/\mathcal{D}_0$ (1sp, lin)")
    axa.plot(t2, D2tot_ratio, color="C3", label=r"$\mathcal{D}_{\rm tot}/\mathcal{D}_0$ (2sp, nl)")
    if D2tot_ratio_lin is not None:
        axa.plot(t2, D2tot_ratio_lin, color="C3", ls="--", label=r"$\mathcal{D}_{\rm tot}/\mathcal{D}_0$ (2sp, lin)")
    ...
    axb.add_artist(leg1)
    axb.legend(handles=case_leg, frameon=False, loc="upper left", borderpad=0.2, labelspacing=0.18, handlelength=1.8)
    ...
    axc.plot(xgrid, s_M, color="C3", ls=":", alpha=0.9, label="Maxwellian")
    axc.set_title("(c) 1 species (sp) vx-slice")
    ...
    if (fa_hist_lin is not None) and (fb_hist_lin is not None) and (dT_ratio_lin is not None):
        ...
        axd.plot(xref, sa_l, color="C2", ls="--", lw=1.85, label=rf"A lin @ $dT/dT_0\approx0.3$ (t={tT_l:.3g})")
        axd.plot(xref, sb_l, color="C3", ls="--", lw=1.85, label=rf"B lin @ $dT/dT_0\approx0.3$ (t={tT_l:.3g})")
    ...
    axd.set_title("(d) 2 species (sp) vx-slices")
...
def run_tests(args) -> None:
    ...
    if jax is not None and jnp is not None:
        ...
        ic1 = build_ic_fig1_1sp_twostream(nmax=nmax_phys, sp=sp1, u=1.5, enforce_nonneg=True)
        ...
        # Linearized: KL entropy to local Maxwellians (same diagnostic as nonlinear).
        D1_lin = np.array([rel_entropy_KL_local_maxwellian_grid_tensor_precomp(dfh[i], sp1, grid_phys) for i in idxs], dtype=np.float64)
        D2_lin = np.array(
            [
                rel_entropy_KL_local_maxwellian_grid_tensor_precomp(dfa_h[i], spa, grid_phys)
                + rel_entropy_KL_local_maxwellian_grid_tensor_precomp(dfb_h[i], spb, grid_phys)
                for i in idxs
            ],
            dtype=np.float64,
        )
        D1_lin_r = D1_lin / (D1_lin[0] if (np.isfinite(D1_lin[0]) and D1_lin[0] > 1e-300) else 1.0)
        D2_lin_r = D2_lin / (D2_lin[0] if (np.isfinite(D2_lin[0]) and D2_lin[0] > 1e-300) else 1.0)
        max_inc_D1_lin = float(np.max(np.maximum(0.0, np.diff(D1_lin_r)))) if D1_lin_r.size > 1 else 0.0
        max_inc_D2_lin = float(np.max(np.maximum(0.0, np.diff(D2_lin_r)))) if D2_lin_r.size > 1 else 0.0
        ...
        physics_rows.append(
            {
                ...
                "max_inc_D1_lin_ratio": max_inc_D1_lin,
                "max_inc_D2_lin_ratio": max_inc_D2_lin,
        ...
        t_plot = idxs * dt_phys
        ax.set_title("Relative-entropy (KL) monotonicity check (sampled)")
        ax.set_xlabel("t")
        ...
        ax.plot(t_plot, D1_lin_r, marker="^", ms=3, ls="--", label=r"$\mathcal{D}/\mathcal{D}_0$ (1sp lin, KL)")
        ax.plot(t_plot, D2_lin_r, marker="v", ms=3, ls="--", label=r"$\mathcal{D}_{\rm tot}/\mathcal{D}_0$ (2sp lin, KL)")
        ax.legend(frameon=False, loc="best")
        fig.savefig(out_dir / "physics_entropy_kl.png", dpi=220, bbox_inches="tight")
        plt.close(fig)

        print(
            f"[tests] physics: max Δ(D1/D10)+={max_inc_D1:.2e}  max Δ(D2/D20)+={max_inc_D2:.2e}  "
            f"max Δ(D1_lin/D1_lin0)+={max_inc_D1_lin:.2e}  max Δ(D2_lin/D2_lin0)+={max_inc_D2_lin:.2e}",
            flush=True,
        )
        ...
        try:
            ...
            ax = fig.add_subplot(111)
            ax.set_title(f"Drifted Maxwellian convergence (u={u_drift}, 1sp; Q={Q_phys}, maxK={maxK_phys})")
            ax.set_xlabel("nmax")
            ax.set_ylabel(r"$\Vert RHS(M_u)\Vert/\Vert M_u\Vert$")
            ax.set_yscale("log")
            ax.grid(True, which="both", alpha=0.3)
            # nmax=1 only spans the collision-invariant subspace (no relaxation modes), so the residual can be
            ...
            fig, axs = plt.subplots(1, 2, figsize=(7.6, 3.2), dpi=180, constrained_layout=True)
            ax0, ax1 = axs[0], axs[1]

            ax0.set_title(f"Linearized: self-adjointness defect (nmax={nmax_phys})")
            ax0.set_xlabel("metric")
            ax0.set_ylabel("value")
            ax0.set_yscale("log")
            ax0.grid(True, which="both", alpha=0.3)
            ax0.bar(["sym_def"], [max(sym_def, 1e-300)])

            # For a physically correct linearized Landau operator, dissipation should be non-positive in the
            # weighted inner product. We plot -<h,Lh>_M >= 0 as a positive quantity on log scale.
            diss_mag1 = max(0.0, -float(diss1))
            diss_mag2 = max(0.0, -float(diss2))
            ax1.set_title("Linearized: dissipation check")
            ax1.set_xlabel("metric")
            ax1.set_ylabel(r"$-\langle h, Lh \rangle_M$ (should be >= 0)")
            ax1.set_yscale("log")
...
def main() -> None:
    ...
    ap.add_argument("--nmax", type=int, default=12, help="Maximum Hermite mode index per velocity dimension.")
    ...
    ap.add_argument("--tmax_2sp", type=float, default=15.0)
    ap.add_argument("--steps_1sp", type=int, default=None)
    ap.add_argument("--steps_2sp", type=int, default=None)
    ap.add_argument("--fig1_ic", choices=["poly4", "twostream", "prl_m2"], default="twostream", help="Fig1 IC family for the 1-species case.")
    ...
    ap.add_argument("--amp1", type=float, default=1.5, help="Fig1 1sp control: for prl_m2 it's the 2nd-moment anisotropy amplitude; for twostream it's the stream separation u in vx/vth; for poly4 it's an anisotropy knob.")
    ap.add_argument("--dT2", type=float, default=0.5, help="Target temperature excess for the hot species in 2sp ICs (positivity-safe).")
    ...
    # Positivity enforcement and filtering intentionally removed for speed-focused runs.
    # Save-to-npz and no-plot modes removed for a leaner Fig1/tests workflow.
    ...
    print(f"[cfg] nu_LB={float(args.nu_LB):.6g}", flush=True)
    ...
    if backend == "numpy":
        ...
        def rhs1_np(f):
            return rhs_ab_np(f, f, T11, use_tt=use_tt, tt_tol=tt_tol, tt_rmax=tt_rmax) + lb_collision_np(f, nu_LB)

        def rhs_pair_np(fa, fb):
            return (
                rhs_ab_np(fa, fb, Tab, use_tt=use_tt, tt_tol=tt_tol, tt_rmax=tt_rmax) + lb_collision_np(fa, nu_LB),
                rhs_ab_np(fb, fa, Tba, use_tt=use_tt, tt_tol=tt_tol, tt_rmax=tt_rmax) + lb_collision_np(fb, nu_LB),
            )
        ...
        jax, jnp = _maybe_import_jax("jax")
        assert jax is not None and jnp is not None and rhs11_jit is not None and rhsab_jit is not None and rhsba_jit is not None
        lb_jit = build_lb_collision_jax(nmax, nu_LB)

        def rhs_self_j(f):
            return rhs11_jit(f, f) + lb_jit(f)

        def rhs_pair_j(fa, fb):
            return rhsab_jit(fa, fb) + lb_jit(fa), rhsba_jit(fb, fa) + lb_jit(fb)
    ...
    def run_case(*, outprefix: str, tag: str, f0_case: np.ndarray, fa0_case: np.ndarray, fb0_case: np.ndarray, do_bench: bool) -> None:
        ...
        if backend == "numpy":
            ...
            t0 = time.perf_counter()
            integrate_1sp_jit_local = integrate_1sp_jit
            integrate_2sp_jit_local = integrate_2sp_jit
            if int(args.progress_chunks) > 0:
                f_hist1_j = integrate_1sp_jax_chunked(
                    rhs_self_j,
                    f0_case,
                    float(args.dt_1sp),
                    steps_1,
                    args.integrator,
                    int(args.progress_chunks),
                    label=f"{tag}:1sp",
                    quiet=quiet,
                )
                fa_hist_j, fb_hist_j = integrate_2sp_jax_chunked(
                    rhs_pair_j,
                    fa0_case,
                    fb0_case,
                    float(args.dt_2sp),
                ...
                f_hist1_j = integrate_1sp_jit_local(jnp.asarray(f0_case), float(args.dt_1sp), steps_1)
                fa_hist_j, fb_hist_j = integrate_2sp_jit_local(jnp.asarray(fa0_case), jnp.asarray(fb0_case), float(args.dt_2sp), steps_2)
            ...
            if do_bench and (int(args.progress_chunks) <= 0):
                ...
                _fh = integrate_1sp_jit_local(jnp.asarray(f0_case), float(args.dt_1sp), steps_1)
                _fah, _fbh = integrate_2sp_jit_local(jnp.asarray(fa0_case), jnp.asarray(fb0_case), float(args.dt_2sp), steps_2)
        ...
        if do_linearized:
            ...
            if lin_method == "tangent":
                if backend == "jax":
                    ...
                    L1_apply_j = make_linearized_rhs_1sp_jax(rhs11_jit, jnp.asarray(fM1_eq_case), nu_LB=nu_LB)
                    J_apply_j = make_linearized_rhs_2sp_jax(rhsab_jit, rhsba_jit, jnp.asarray(faM_eq_case), jnp.asarray(fbM_eq_case), nu_LB=nu_LB)
                    ...
                    assert fM1_eq_case is not None and faM_eq_case is not None and fbM_eq_case is not None
                    L1_apply_np = make_linearized_rhs_1sp_numpy(T11, fM1_eq_case, use_tt=use_tt, tt_tol=tt_tol, tt_rmax=tt_rmax, nu_LB=nu_LB)
                    J_apply_np = make_linearized_rhs_2sp_numpy(Tab, Tba, faM_eq_case, fbM_eq_case, use_tt=use_tt, tt_tol=tt_tol, tt_rmax=tt_rmax, nu_LB=nu_LB)
        ...
        if (f_hist1_lin is not None) and (fa_hist_lin is not None) and (fb_hist_lin is not None):
            # Linearized overlay: report the *same* diagnostic as the nonlinear run (relative entropy / KL).
            # For exact collision dynamics, the local-Maxwellian reference is time-independent because the
            # collision invariants (n, P, W) are conserved; numerically this also provides a robust check.
            D1l = np.array([rel_entropy_KL_local_maxwellian_grid_tensor_precomp(f_hist1_lin[i], sp1, grid) for i in range(len(t1))], dtype=np.float64)
            D2totl = np.array(
                [
                    rel_entropy_KL_local_maxwellian_grid_tensor_precomp(fa_hist_lin[i], spa, grid)
                    + rel_entropy_KL_local_maxwellian_grid_tensor_precomp(fb_hist_lin[i], spb, grid)
                    for i in range(len(t2))
                ],
                dtype=np.float64,
            )
            D10l = float(D1l[0]) if (np.isfinite(D1l[0]) and D1l[0] > 1e-300) else 1.0
            D20l = float(D2totl[0]) if (np.isfinite(D2totl[0]) and D2totl[0] > 1e-300) else 1.0
            D1_ratio_lin = D1l / D10l
            D2tot_ratio_lin = D2totl / D20l
